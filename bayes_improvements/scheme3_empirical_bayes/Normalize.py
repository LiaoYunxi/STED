...
def ModelEvaluateBayes(topic_cell_mat, topic_celltype_df, cell_celltype_list, celltype_num_dict, model_dir, custom_prior=None):
    """
    Modified ModelEvaluateBayes (Scheme 3): Empirical Bayes.
    Accepts an optional `custom_prior` array to allow iterative updating 
    based on bulk predictions.
    """
    # bayes
    ...
    if custom_prior is not None:
        celltype_prob_array = np.array(custom_prior)
    else:
        celltype_prob_list = []

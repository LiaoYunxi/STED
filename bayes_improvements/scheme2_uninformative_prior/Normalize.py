...
def ModelEvaluateBayes(topic_cell_mat, topic_celltype_df, cell_celltype_list, celltype_num_dict, model_dir):
    """
    Modified ModelEvaluateBayes (Scheme 2): Uninformative Prior.
    Replaces reference-based priors with a uniform distribution (1/N) 
    to remove anchoring effect for rare cells.
    """
    # bayes
    topic_prob_array = np.array([topic_celltype_df.sum(axis=1)]).T
    topic_prob_array = topic_prob_array / topic_prob_array.sum()
    
    # OLD: celltype_prob_list.append(celltype_num_dict[celltype] / len(cell_celltype_list))
    # NEW: Uniform prior
    n_celltypes = len(celltype_num_dict)
    celltype_prob_array = np.array([1.0 / n_celltypes] * n_celltypes)

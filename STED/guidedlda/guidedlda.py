...
#from guidedlda import guidedutils
from .guidedutils import *
import random
...
class GuidedLDA:
    ...
    def compute_umass_coherence(self, X,top_n=30, epsilon=1e-12):
        if not isinstance(X, (csc_matrix, csr_matrix)):
            X = csc_matrix(X)
        ...
        # 获取每个主题的top_n个词
        top_words = get_top_words(topic_word, top_n)
        
        # 将文档-词矩阵转换为二值化矩阵
        binary_matrix = (X > 0).astype(int)
        
        # 计算词频（每个词出现在多少篇文档中）
        word_doc_freq = binary_matrix.sum(axis=0).A1  # A1将结果转换为一维数组
        
        # 计算词对共现频率
        pair_doc_freq = Counter()
        for doc in binary_matrix:
            # 获取文档中非零元素的索引
            if isinstance(doc, (csc_matrix, csr_matrix)):
                word_indices = doc.nonzero()[1]  # 对于行向量，nonzero()返回的是(行索引, 列索引)，我们只需要列索引
            else:
                word_indices = np.where(doc.toarray().flatten() > 0)[0]

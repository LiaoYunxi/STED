...
class ClassTfidfTransformer(TfidfTransformer):
    ...
    Arguments:
        ...
        seed_multiplier: The value with which the idf values of the words in `seed_words` are multiplied. #anchor_strength
    """
...
#     c-TF-IDF is calculated by combining all documents per class into one document.
#     """
...
#         self.seed_multiplier = seed_multiplier
#         super().__init__(use_idf=use_idf, smooth_idf=smooth_idf)

#     def fit(self, X: sp.csr_matrix, multiplier: np.ndarray = None):
#         """Learn the idf vector (global term weights) for each class."""
...
#             # Calculate the frequency of words across all classes
#             df = np.squeeze(np.asarray((X > 0).sum(axis=0)))
#             avg_nr_samples = X.sum(axis=1).mean()

#             # BM25-inspired weighting or c-TF-IDF standard weighting
#             if self.bm25_weighting:
#                 idf = np.log(1 + ((avg_nr_samples - df + 0.5) / (df + 0.5)))
#             else:
#                 denominator = np.maximum(df, np.finfo(float).eps)  # 避免零除
#                 idf = np.log((avg_nr_samples / denominator) + 1)

#             # Apply seed_multiplier to seed_words
#             if self.seed_words:
#                 seed_indices = [idx for idx, term in enumerate(self.get_feature_names_out()) if term in self.seed_words]
#                 idf[seed_indices] *= self.seed_multiplier

#             # Apply multiplier for custom adjustments if provided
#             if multiplier is not None:
#                 idf *= multiplier
...
#             # Reduce frequent words by taking the square root
#             if self.reduce_frequent_words:
#                 X.data = np.sqrt(X.data)

...
class gsPreProcessing():
    ...
    def align(self,sc_exp,do_align=True,do_transform=True):
        if self.target_df is None:
            self.target_df = self.mix_raw

        if do_align:
            # x = np.array(sc_exp.mean(0)).flatten()
            mean_x = np.mean(sc_exp.todense(), axis=0)
            std_x = np.std(sc_exp.todense(), axis=0)
            # target_df = np.array(self.target_df.todense())
            if do_transform:
                target_df = (self.target_df - mean_x) / std_x
                # x_mean, x_std = np.mean(x), np.std(x)
                # for i in range(target_df.shape[0]):
                #     y = np.array(target_df[i,:]).flatten()
                #     y_mean, y_std = np.mean(y), np.std(y)
                #     target_df[i,:] = (y - y_mean) * (x_std / y_std) + x_mean
            else:
                x_sorted = np.sort(x)
                for i in range(target_df.shape[0]):
                    y = np.array(target_df[i,:]).flatten()
                    y_ranks = np.argsort(np.argsort(y))
                    target_df[i,:] = x_sorted[y_ranks]
            self.target_df = csr_matrix(target_df)
